...
import { Map, Navigation, Shield, AlertTriangle, User, Clock, Radio, ShieldAlert } from 'lucide-react';
...
export function TacticalMap({ incidents, units }: TacticalMapProps) {
  const { t, language } = useLanguage();
  const [selectedEntity, setSelectedEntity] = useState<string | null>(null);
  const [entities, setEntities] = useState<MapEntity[]>([]);

  // Simple projection for demo: Lat 31.74 - 31.76 -> Y 100 - 0
  // Lon 34.97 - 35.00 -> X 0 - 100
  const LAT_MIN = 31.74;
  const LAT_MAX = 31.76;
  const LON_MIN = 34.97;
  const LON_MAX = 35.00;
  ...
  useEffect(() => {
    // If incidents or units are undefined, skip processing
    ...
    safeIncidents.forEach(inc => {
      // If no location, generate one based on ID hash for stability
      let pos = { x: 50, y: 50 };
      if (inc.location) {
    ...
    safeUnits.forEach(unit => {
      let pos = { x: 50, y: 50 };
      // Check if unit has coordinates (if I updated Unit interface)
      // Assuming unit.location is string "lat, lng" if mapped from backend
